 ...
 *
 */
public class YCSBServer extends DurabilityCoordinator {//DefaultRecoverable {

	static FSTConfiguration conf = FSTConfiguration.createDefaultConfiguration();
	
    private static final boolean _debug = false;
    private YCSBNamespace mTables;
    ...
    private YCSBServer(int id) {
        this.mTables = new YCSBNamespace();
                    ...
                    switch (aRequest.getEntity()) {
                        case RECORD: // ##### entity: record #####
                            ...
                            for(int i = 0;i < 1000; i++)
                            	mTables.get(aRequest.getTable()).put(""+(Integer.parseInt(aRequest.getKey())+i), aRequest.getValues());
...
//		System.out.println("RETURNING REPLY");
    ...
    public void installSnapshot(byte[] state) {
    	mTables = (YCSBNamespace)conf.asObject(state);
//    	
//        try {
//            ByteArrayInputStream bis = new ByteArrayInputStream(state);
//            ObjectInput in = new ObjectInputStream(bis);
//            mTables = (YCSBNamespace) in.readObject();
//            in.close();
//            bis.close();
...
//        }
    ...
    public byte[] getSnapshot() {
    	byte barray[] = conf.asByteArray(mTables);
    	return barray;
//        try {

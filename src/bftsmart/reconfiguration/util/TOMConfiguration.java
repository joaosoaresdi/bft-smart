...
public class TOMConfiguration extends Configuration {
    ...
    protected void init() {
        ...
        try {
            ...
            //Added by JSoares for Sharded State/ChkPnt Transfer
            s = (String) configs.remove("system.state.shardsize");
            if (s == null) {
            	shardSize = 1024;
            } else {
            	shardSize = Integer.parseInt(s);
            }

            s = (String) configs.remove("system.state.mktree.hashAlgorithm");
            if(s == null) {
            	mrklTreeHashAlgo = "SHA-256";
            } else {
            	mrklTreeHashAlgo = s;
            }
	...
	public MessageDigest getMrklTreeMessageDigest() {
		try {
			return MessageDigest.getInstance(mrklTreeHashAlgo);

...
public class CSTState implements ApplicationState {
    ...
    public CSTState(byte[] state, byte[] hashCheckpoint, CommandsInfo[] logLower, byte[] hashLogLower,
    	...
    	this.state = state;
        this.logUpperhHash = hashLogUpper;
        this.logLowerHash = hashLogLower;
        this.checkpointHash = hashCheckpoint;
    ...
    public CommandsInfo getMessageBatch(int cid) {
    	if (cid >= checkpointCID && cid <= lastCID) {
        	// modified by JSoares correct outOfBounds index (-1)
    ...
    public byte[] getLogUpperHash() {
        return logUpperhHash;
    }

    public byte[] getLogLowerHash() {
        return logLowerHash;
    }

...
public class StandardStateManager extends StateManager {
    ...
    protected void requestState() {
    	logger.debug("requeste State from StandardStateManager");
        ...
        logger.info("I ("+ SVController.getStaticConf().getProcessId() + ") just sent a request to the other replicas for the state up to CID " + waitingCID);
    ...
    public void SMReplyDeliver(SMMessage msg, boolean isBFT) {
        ...
        if (SVController.getStaticConf().isStateTransferEnabled()) {
            if (waitingCID != -1 && msg.getCID() == waitingCID) {
                ...
                if (!appStateOnly) {
                    receivedRegencies.put(msg.getSender(), msg.getRegency());
                    receivedLeaders.put(msg.getSender(), msg.getLeader());
                    receivedViews.put(msg.getSender(), msg.getView());
                    receivedProofs.put(msg.getSender(), msg.getState().getCertifiedDecision(SVController));
                ...
                if (msg.getSender() == replica && msg.getState().getSerializedState() != null) {
                    ...
                    chkpntState = msg.getState();
                    if (stateTimer != null) {
                        stateTimer.cancel();
                    }
                }

                receivedStates.put(msg.getSender(), msg.getState());
                ...
                if (enoughReplies()) {
                    ...
                    if (chkpntState != null) {
                        byte[] hash = null;
                        hash = tomLayer.computeHash(chkpntState.getSerializedState());
    ...
    private ApplicationState getOtherReplicaState() {
        ...
        for (int process : processes) {
            if (process == replica) {
                ...
                ApplicationState otherState = receivedStates.get(process);
                if (otherState != null) {
                    return otherState;
                }
            }

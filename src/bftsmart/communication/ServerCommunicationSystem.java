...
public class ServerCommunicationSystem extends Thread{
        ...
        while (doWork) {
            try {
                if (count % 1000 == 0 && count > 0) {
                    logger.info("After " + count + " messages, inQueue size=" + inQueue.size());
                }
    ...
    public void send(int[] targets, SystemMessage sm) {
        if (sm instanceof TOMMessage) {
            logger.info("client response----------> " + sm);
	...
	public String toString() {
		return "ServerCommunicationSystem [logger=" + logger + ", doWork=" + doWork + ", MESSAGE_WAIT_TIME="
				+ MESSAGE_WAIT_TIME + ", inQueue=" + inQueue + ", messageHandler=" + messageHandler + ", serversConn="
				+ serversConn + ", clientsConn=" + clientsConn + ", controller=" + controller + "]";
	}

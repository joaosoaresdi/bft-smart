...
public final class Acceptor {
	...
	public String toString() {
		return "Acceptor [logger=" + logger + ", me=" + me + ", executionManager=" + executionManager + ", factory="
				+ factory + ", communication=" + communication + ", tomLayer=" + tomLayer + ", controller=" + controller
    ...
    public void proposeReceived(Epoch epoch, ConsensusMessage msg) {
    	...
    	if (msg.getSender() == executionManager.getCurrentLeader() // Is the replica the leader?
    		...
    		logger.info("Propose received is not from the expected leader (will be ignored)");
    	}
    }
    ...
    private void executePropose(Epoch epoch, byte[] value) {
        ...
        if(epoch.propValue == null) { //only accept one propose per epoch
            ...
            logger.info("I have written value " + Arrays.toString(epoch.propValueHash) + " in consensus instance " + cid + " with timestamp " + epoch.getConsensus().getEts());
            /*****************************************/
            ...
            if (epoch.deserializedPropValue != null && !epoch.isWriteSent()) {
                ...
                if(controller.getStaticConf().isBFT()){
                    logger.info("Sending WRITE for " + cid);
                    ...
                    logger.info("WRITE computed for " + cid);
                 	...
                 	/**** LEADER CHANGE CODE! ******/
                        logger.info("[CFT Mode] Setting consensus " + cid + " QuorumWrite tiemstamp to " + epoch.getConsensus().getEts() + " and value " + Arrays.toString(epoch.propValueHash));
    ...
    private void writeReceived(Epoch epoch, int a, byte[] value) {
        int cid = epoch.getConsensus().getId();
        logger.info("WRITE from " + a + " for consensus " + cid);
    ...
    private void computeWrite(int cid, Epoch epoch, byte[] value) {
        ...
        logger.info("I have " + writeAccepted +
        ...
        if (writeAccepted > controller.getQuorum() && Arrays.equals(value, epoch.propValueHash)) {
            ...
            if (!epoch.isAcceptSent()) {
                ...
                /**** LEADER CHANGE CODE! ******/
                logger.info("Setting consensus " + cid + " QuorumWrite tiemstamp to " + epoch.getConsensus().getEts() + " and value " + Arrays.toString(value));
                ...
                if (Arrays.equals(cm.getValue(), value)) { //make sure the ACCEPT message generated upon receiving the PROPOSE message
                    ...
                    logger.info("Speculative ACCEPT message for consensus {} matches the written value, sending it to the other replicas", cid);
                    ...
                    proofExecutor.submit(() -> {
                        ...
                        logger.info("Creating cryptographic proof for the correct ACCEPT message from consensus " + cid);
            ...
            proofExecutor.submit(() -> {
                ...
                // Create a cryptographic proof for this ACCEPT message
                logger.info("Creating cryptographic proof for speculative ACCEPT message from consensus " + cid);
    ...
    private void computeAccept(int cid, Epoch epoch, byte[] value) {
        logger.trace(cid + "---" + epoch.toString() + "---" + Arrays.toString(value));

        logger.info("I have " + epoch.countAccept(value) +

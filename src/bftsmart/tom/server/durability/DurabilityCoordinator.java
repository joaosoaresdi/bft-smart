...
public abstract class DurabilityCoordinator implements Recoverable, BatchExecutable {
	...
	private byte[][] executeBatch(byte[][] commands, MessageContext[] msgCtx, boolean noop) {
		...
		// replicas is
		// supposed to take a checkpoint, so the replica will only execute the command
		// and
		// return the replies
...
//			cid = firstHalfMsgCtx[checkpointIndex].getConsensusId();
			if (cid % globalCheckpointPeriod == replicaCkpIndex && lastCkpCID < cid) {
				...
				logger.debug("Storing message batch in the state log for consensus "
								+ cid);
				boolean writeToBeforeCkpLog = firstHalfMsgCtx[firstHalfMsgCtx.length - 1]
						.getConsensusId() < globalCheckpointPeriod;
				saveCommands(commands, msgCtx, true, writeToBeforeCkpLog);
			}
			...
			if (secondHalf.length > 0) {
				...
				logger.debug("Storing message batch in the state log for consensus "
								+ cid);
				boolean writeToBeforeCkpLog = secondHalfMsgCtx[secondHalfMsgCtx.length - 1]
						.getConsensusId() < globalCheckpointPeriod;
	 ...
	 *         by this command batch, it returns -1
	 */
	...
	public ApplicationState getState(int cid, boolean sendState) {
		logLock.lock();
		ApplicationState ret = new DefaultApplicationState();
		ret.setSerializedState(getSnapshot());
		logLock.unlock();
		return ret;
	}
...
//					logger.info("Executing batch {} requests for CID " + cid, cmdInfo);
			...
			logger.info("State successfuly installed");
			stateLock.unlock();
	 ...
	 * @param msgCtx
	 */
	private void saveCommands(byte[][] commands, MessageContext[] msgCtx, boolean writeToRegularLog,
			boolean writeToBeforeCkpLog) {
		if (!config.isToLog())
			return;
		...
		for (int i = 0; i <= msgCtx.length; i++) {
			if (i == msgCtx.length) {
				// the batch command contains only one command or it is the last position of the
				// array
...
//				System.out.println("#### USING SHARDED CST PROTOCOL #####");
	...
	private CSTState getShardedState(ShardedCSTRequest cstRequest) {
		if (cstRequest.getCID() < globalCheckpointPeriod) {
			return log.getStateBeforeGlobal(cstRequest);
		} else {
